...
def get_manager_from_dn(distinguished_name):
    ...
    if result is not None and result.manager is not None and result.manager.value is not None:
        mgr_email = get_object(result.manager.value, ["mail"])
        if mgr_email is not None and mgr_email.mail is not None and mgr_email.mail.values != []:
